...
const OT_details = () => {
    ...
    const submit = async (e) => {
        ...
        try{
            ...
            const loginResponse = await axios.post("http://localhost:3001/api/Addresearch", headers,{ params: {
                    name,
                    details
                }},{
                credentials: 'include',
            ...
            <Grid container justify="center" direction='row'  spacing={10}>
                ...
                <Grid item md={8}>
                    <Container  fixed>
                        ...
                        <div className="table">
                            {listData.map((current, i) => (
                                <>
                                <label>Research ID :{current.name}</label><br/>
                                <label>Research Name :{current.name}</label><br/>
                                <label>Researcher :username</label><br/>
                                <label>Details :{current.name}</label><br/>
                                </>
                            ))}
